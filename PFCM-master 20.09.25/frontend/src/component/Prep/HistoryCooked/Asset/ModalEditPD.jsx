...
const ModalEditPD = ({ open, onClose, data, onSuccess }) => {
  ...
  const fetchMaterialName = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/fetchRawMatName`, { params: { mat } });
  ...
  const fetchProduction = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/fetchProduction`, { params: { mat } });
      if (response.data.success) {
