...
const DeleteProdRawmat = ({ isOpen, onClose, onSuccess, data }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `${API_URL}/api/delete-prod-rawmat/${mat}`
      );
  ...
  return (
    <div className="modal-overlay" onClick={handleClose}>
      ...
      >
        <div>
          <h1 className="text-rose-500 text-2xl">ลบแผนการผลิตของวัตถุดิบทั้งหมด !!</h1>
          <p className="text-gray-700">
            คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลผนการผลิตของวัตถุดิบนี้ ?
          </p>
        </div>
...
export default DeleteProdRawmat;
