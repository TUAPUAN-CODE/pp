...
const EditRawmatGroup = ({ isOpen, onClose, onSuccess, data }) => {
  ...
  const [rmGroupId, setRmGroupId] = useState(data?.rm_group_id || "");
  ...
  const [times, setTimes] = useState({
    prep_to_cold: data?.prep_to_cold || "",
    prep_to_pack: data?.prep_to_pack || "",
    cold_to_pack: data?.cold_to_pack || "",
    cold: data?.cold || "",
    rework: data?.rework || "",
  });
  ...
  useEffect(() => {
    if (isOpen) {
      setRmGroupId(data?.rm_group_id || "");
      setRawmatGroupName(data?.rm_group_name || "");
      setSelectedRawmatType(data?.rm_type_id || "");
      setTimes({
        prep_to_cold: data?.prep_to_cold || "",
        prep_to_pack: data?.prep_to_pack || "",
        cold_to_pack: data?.cold_to_pack || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${API_URL}/api/update/rawmat-group`, rawmatGroupData);
      
      if (response.data.success) {
        // แสดง alert แทนการใช้ alert() ของ browser
        showAlert("แก้ไขกลุ่มวัตถุดิบสำเร็จ!");
  ...
  return (
    <div className="modal-overlay" onClick={handleClose}>
      ...
      >
        <h1 className="pb-5 text-2xl">แก้ไขกลุ่มวัตถุดิบ</h1>
...
export default EditRawmatGroup;