      ...
      <DialogContent>
        ...
        <Stack spacing={1}>
          ...
          <Typography color="rgba(0, 0, 0, 0.6)">
            ...
            {selectedPlans
              .map(
                (plan) => `${plan.code} (${plan.doc_no} - ${plan.line_name})`
              )
              .join(", ")}
          </Typography>
...
const ModalEditPD = ({ open, onClose, data }) => {
  ...
  const handlePlanSelect = (plan) => {
    if (
      !selectedPlans.some(
        (selectedPlan) => selectedPlan.prod_id === plan.prod_id
      )
    ) {
  ...
  const handlePlanDelete = (planToDelete) => {
    setSelectedPlans((plans) =>
      plans.filter((plan) => plan.prod_id !== planToDelete.prod_id)
    );
  };

  const handleConfirm = () => {
    if (selectedPlans.length === 0) {
      // || !deliveryLocation || !operator
