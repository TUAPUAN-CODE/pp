...
const ConfirmProdModal = ({ open, onClose, material, materialName, batch, mapping_id, tro_id, ntray, weight_per_tro, rmfp_id, onSuccess, onParentClose }) => {
 ...
 const handleConfirm = async () => {
  const line_id = Number(localStorage.getItem("line_id")); // ดึงจาก localStorage และแปลงเป็นตัวเลข
  ...
  const payload = {
    ...
    ntray,
    batch_after: batch,
    line_id // เพิ่ม line_id เข้า payload
  };

  try {
    const response = await axios.put(`${API_URL}/api/pack/Add/request/rm/TrolleyMapping`, payload);
...
const ModalEditPD = ({ open, onClose, data, onSuccess }) => {
  ...
  const { batch_after, mat, rmfp_id, mapping_id,request_rm_id } = data || {};

  useEffect(() => {
    if (data) {
      setMaxWeight(parseFloat(data.weight_RM || 0));
      setErrorMessage("");
    }
  }, [data]);
 ...
 const handleConfirm = async () => {
  try {
    const response = await axios.delete(`${API_URL}/api/delete/request/RM/${request_rm_id}`);
    if (response.status === 200) {
      onSuccess();
      onClose();
      onParentClose();
      ...
      console.error("Error while deleting data:", response.status);
    }
  } catch (error) {
  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
        <DialogContent>
          <Typography variant="h6" style={{ fontSize: "18px", color: "#787878" }} mb={2}> คุณต้องการยกเลิกคำขอวัตถุดิบใช่ไหม </Typography>
          ...
          <Stack spacing={2}>
            ...
            <Typography>Batch: {request_rm_id}</Typography>
