...
import ParentComponentChill4 from '../Move/Chill4/ParentComponentChill4';

const API_URL = import.meta.env.VITE_API_URL;

const Modalchill4 = ({ open, onClose, onNext, data, rmfp_id, CookedDateTime, dest }) => {
  console.log("Modalchill4 - ได้รับข้อมูล:", { data, rmfp_id, CookedDateTime, dest });

  // ฟังก์ชันจัดการเมื่อได้รับข้อมูลจาก ParentComponentChill4
  const handleSlotSelect = (option, slotData) => {
    console.log("Modalchill4 - ได้รับข้อมูลจาก ParentComponentChill4:", option, slotData);

    // ส่งข้อมูลไปยัง parent component
    if (onNext && typeof onNext === 'function') {
      console.log("Modalchill4 - ส่งข้อมูลไปยัง SlotModal:", option, slotData);
      onNext("SELECT_TROLLEY", slotData);
    } else {
      console.log("Modalchill4 - ไม่มี onNext หรือไม่ใช่ฟังก์ชัน:", onNext);
    }
  };
  ...
  return (
    <Dialog
      ...
      onClose={(e, reason) => {
        ...
        console.log("Modalchill4 - ปิด modal");
        onClose();
      }}
      fullWidth
      maxWidth="lg"
    >
        ...
        <Toolbar sx={{ minHeight: '50px', px: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
              เลือกรถเข็นปลายทาง - {data?.ColdMove || "Chill 4"}
            </Typography>
          </Box>
      ...
      <DialogContent>
        ...
        <ParentComponentChill4 onSelectOption={handleSlotSelect} />
      </DialogContent>
