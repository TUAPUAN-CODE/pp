...
import ParentComponentAnte from '../Move/Ante/ParentComponentAnte';

const API_URL = import.meta.env.VITE_API_URL;

const ModalAnte = ({ open, onClose, onNext, data, rmfp_id, CookedDateTime, dest }) => {
  console.log("ModalAnte - ได้รับข้อมูล:", { data, rmfp_id, CookedDateTime, dest });

  // ฟังก์ชันจัดการเมื่อได้รับข้อมูลจาก ParentComponentAnte
  const handleSlotSelect = (option, slotData) => {
    console.log("ModalAnte - ได้รับข้อมูลจาก ParentComponentAnte:", option, slotData);

    // ส่งข้อมูลไปยัง parent component
    if (onNext && typeof onNext === 'function') {
      console.log("ModalAnte - ส่งข้อมูลไปยัง SlotModal:", option, slotData);
      onNext("SELECT_TROLLEY", slotData);
    } else {
      console.log("ModalAnte - ไม่มี onNext หรือไม่ใช่ฟังก์ชัน:", onNext);
    }
  };
  ...
  return (
    <Dialog
      ...
      onClose={(e, reason) => {
        ...
        console.log("ModalAnte - ปิด modal");
        onClose();
      }}
      fullWidth
      maxWidth="lg"
    >
        ...
        <Toolbar sx={{ minHeight: '50px', px: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
              เลือกรถเข็นปลายทาง - {data?.ColdMove || "Ante"}
            </Typography>
          </Box>
      ...
      <DialogContent>
        ...
        <ParentComponentAnte onSelectOption={handleSlotSelect} />
      </DialogContent>
