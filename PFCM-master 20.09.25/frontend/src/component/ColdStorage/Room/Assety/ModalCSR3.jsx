...
import ParentComponentCSR3 from '../Move/CSR3/ParentComponentCSR3';

const API_URL = import.meta.env.VITE_API_URL;

const ModalCSR3 = ({ open, onClose, onNext, data, rmfp_id, CookedDateTime, dest }) => {
  console.log("ModalCSR3 - ได้รับข้อมูล:", { data, rmfp_id, CookedDateTime, dest });

  // ฟังก์ชันจัดการเมื่อได้รับข้อมูลจาก ParentComponentCSR3
  const handleSlotSelect = (option, slotData) => {
    console.log("ModalCSR3 - ได้รับข้อมูลจาก ParentComponentCSR3:", option, slotData);

    // ส่งข้อมูลไปยัง parent component
    if (onNext && typeof onNext === 'function') {
      console.log("ModalCSR3 - ส่งข้อมูลไปยัง SlotModal:", option, slotData);
      onNext("SELECT_TROLLEY", slotData);
    } else {
      console.log("ModalCSR3 - ไม่มี onNext หรือไม่ใช่ฟังก์ชัน:", onNext);
    }
  };
  ...
  return (
    <Dialog
      ...
      onClose={(e, reason) => {
        ...
        console.log("ModalCSR3 - ปิด modal");
        onClose();
      }}
      fullWidth
      maxWidth="lg"
        ...
        <Toolbar sx={{ minHeight: '50px', px: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
              เลือกรถเข็นปลายทาง - {data?.ColdMove || "CSR3"}
            </Typography>
          </Box>
