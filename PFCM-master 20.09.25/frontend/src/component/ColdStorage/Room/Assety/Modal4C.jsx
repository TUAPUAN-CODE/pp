...
import ParentComponent4C from '../Move/4C/ParentComponent4C';

const API_URL = import.meta.env.VITE_API_URL;

const Modal4C = ({ open, onClose, onNext, data, rmfp_id, CookedDateTime, dest }) => {
  console.log("Modal4C - ได้รับข้อมูล:", { data, rmfp_id, CookedDateTime, dest });

  // ฟังก์ชันจัดการเมื่อได้รับข้อมูลจาก ParentComponent4C
  const handleSlotSelect = (option, slotData) => {
    console.log("Modal4C - ได้รับข้อมูลจาก ParentComponent4C:", option, slotData);

    // ส่งข้อมูลไปยัง parent component
    if (onNext && typeof onNext === 'function') {
      console.log("Modal4C - ส่งข้อมูลไปยัง SlotModal:", option, slotData);
      onNext("SELECT_TROLLEY", slotData);
    } else {
      console.log("Modal4C - ไม่มี onNext หรือไม่ใช่ฟังก์ชัน:", onNext);
    }
  };
  ...
  return (
    ...
    >
        ...
        <Toolbar sx={{ minHeight: '50px', px: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
              เลือกรถเข็นปลายทาง - {data?.ColdMove || "4C"}
            </Typography>
          </Box>
      ...
      <DialogContent>
        ...
        <ParentComponent4C onSelectOption={handleSlotSelect} />
      </DialogContent>
