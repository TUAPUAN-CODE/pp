...
import ParkingLayoutAnte from "./RoomAnte";
import ScanTrolley from "../../Asset/ScanTrolley";
import CheckTrolley from "../../Asset/CheckTrolley";
const API_URL = import.meta.env.VITE_API_URL;

const ParentComponentAnte = ({ onSelectOption }) => {
  console.log("ParentComponentAnte - ถูกเรียกใช้ด้วย onSelectOption:", onSelectOption);
  ...
  const handleSlotClick = async (slotData) => {
    console.log("ParentComponentAnte - ช่องจอดถูกคลิก:", slotData);
    ...
    try {
      console.log("ParentComponentAnte - กำลังอัปเดตสถานะช่องจอด:", {
      ...
      console.log("ParentComponentAnte - ผลการตอบกลับจาก API:", response);
      ...
      console.log("ParentComponentAnte - ข้อมูลจาก API:", data);
  
      if (!data.success) {
        throw new Error(`Failed to update slot reservation: ${data.message || "Unknown error"}`);
      }
  
      console.log("ParentComponentAnte - อัปเดตสถานะช่องจอดสำเร็จ");
      ...
      // ส่งข้อมูลกลับไปยัง ModalAnte โดยตรง
      if (onSelectOption && typeof onSelectOption === 'function') {
        console.log("ParentComponentAnte - ส่งข้อมูลกลับไปยัง ModalAnte:", slotData);
        onSelectOption("SELECT_SLOT", slotData);
      } else {
        console.log("ParentComponentAnte - ไม่สามารถส่งข้อมูลได้ - onSelectOption ไม่ใช่ฟังก์ชัน:", onSelectOption);
      }
    } catch (error) {
  ...
  const handleOptionSelect = (option, slotData) => {
    // เรียกใช้ onSelectOption ที่ส่งมาจาก ModalAnte
    if (onSelectOption && typeof onSelectOption === 'function') {
  ...
  return (
    <div className="p-0">
      <ParkingLayoutAnte onSlotClick={handleSlotClick} />
...
export default ParentComponentAnte;