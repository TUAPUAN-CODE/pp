...
const ParentComponentCSR3 = ({ onSelectOption }) => {
  console.log("ParentComponentCSR3 - ถูกเรียกใช้ด้วย onSelectOption:", onSelectOption);
  ...
  const handleSlotClick = async (slotData) => {
    console.log("ParentComponentCSR3 - ช่องจอดถูกคลิก:", slotData);
    ...
    try {
      console.log("ParentComponentCSR3 - กำลังอัปเดตสถานะช่องจอด:", {
      ...
      console.log("ParentComponentCSR3 - ผลการตอบกลับจาก API:", response);
      ...
      console.log("ParentComponentCSR3 - ข้อมูลจาก API:", data);
  
      if (!data.success) {
        throw new Error(`Failed to update slot reservation: ${data.message || "Unknown error"}`);
      }
  
      console.log("ParentComponentCSR3 - อัปเดตสถานะช่องจอดสำเร็จ");
      ...
      // ส่งข้อมูลกลับไปยัง ModalCSR3 โดยตรง
      if (onSelectOption && typeof onSelectOption === 'function') {
        console.log("ParentComponentCSR3 - ส่งข้อมูลกลับไปยัง ModalCSR3:", slotData);
        onSelectOption("SELECT_SLOT", slotData);
      } else {
        console.log("ParentComponentCSR3 - ไม่สามารถส่งข้อมูลได้ - onSelectOption ไม่ใช่ฟังก์ชัน:", onSelectOption);
      }
    } catch (error) {
  ...
  const handleOptionSelect = (option, slotData) => {
    // เรียกใช้ onSelectOption ที่ส่งมาจาก ModalCSR3
    if (onSelectOption && typeof onSelectOption === 'function') {
