...
import ParkingLayoutChill4 from "./RoomChill4";
import ScanTrolley from "../../Asset/ScanTrolley";
import CheckTrolley from "../../Asset/CheckTrolley";
const API_URL = import.meta.env.VITE_API_URL;

const ParentComponentChill4 = ({ onSelectOption }) => {
  console.log("ParentComponentChill4 - ถูกเรียกใช้ด้วย onSelectOption:", onSelectOption);
  ...
  const handleSlotClick = async (slotData) => {
    console.log("ParentComponentChill4 - ช่องจอดถูกคลิก:", slotData);
    ...
    try {
      console.log("ParentComponentChill4 - กำลังอัปเดตสถานะช่องจอด:", {
      ...
      console.log("ParentComponentChill4 - ผลการตอบกลับจาก API:", response);
      ...
      console.log("ParentComponentChill4 - ข้อมูลจาก API:", data);
  
      if (!data.success) {
        throw new Error(`Failed to update slot reservation: ${data.message || "Unknown error"}`);
      }
  
      console.log("ParentComponentChill4 - อัปเดตสถานะช่องจอดสำเร็จ");
      ...
      // ส่งข้อมูลกลับไปยัง Modalchill4 โดยตรง
      if (onSelectOption && typeof onSelectOption === 'function') {
        console.log("ParentComponentChill4 - ส่งข้อมูลกลับไปยัง Modalchill4:", slotData);
        onSelectOption("SELECT_SLOT", slotData);
      } else {
        console.log("ParentComponentChill4 - ไม่สามารถส่งข้อมูลได้ - onSelectOption ไม่ใช่ฟังก์ชัน:", onSelectOption);
      }
    } catch (error) {
  ...
  const handleOptionSelect = (option, slotData) => {
    // เรียกใช้ onSelectOption ที่ส่งมาจาก Modalchill4
    if (onSelectOption && typeof onSelectOption === 'function') {
  ...
  return (
    <div className="p-0">
      <ParkingLayoutChill4 onSlotClick={handleSlotClick} />
...
export default ParentComponentChill4;