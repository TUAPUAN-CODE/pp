...
const ParentComponentChill2 = ({ onSelectOption }) => {
  console.log("ParentComponentChill2 - ถูกเรียกใช้ด้วย onSelectOption:", onSelectOption);
  ...
  const handleSlotClick = async (slotData) => {
    console.log("ParentComponentChill2 - ช่องจอดถูกคลิก:", slotData);
    ...
    try {
      console.log("ParentComponentChill2 - กำลังอัปเดตสถานะช่องจอด:", {
      ...
      console.log("ParentComponentChill2 - ผลการตอบกลับจาก API:", response);
      ...
      console.log("ParentComponentChill2 - ข้อมูลจาก API:", data);
  
      if (!data.success) {
        throw new Error(`Failed to update slot reservation: ${data.message || "Unknown error"}`);
      }
  
      console.log("ParentComponentChill2 - อัปเดตสถานะช่องจอดสำเร็จ");
      ...
      // ส่งข้อมูลกลับไปยัง Modalchill2 โดยตรง
      if (onSelectOption && typeof onSelectOption === 'function') {
        console.log("ParentComponentChill2 - ส่งข้อมูลกลับไปยัง Modalchill2:", slotData);
        onSelectOption("SELECT_SLOT", slotData);
      } else {
        console.log("ParentComponentChill2 - ไม่สามารถส่งข้อมูลได้ - onSelectOption ไม่ใช่ฟังก์ชัน:", onSelectOption);
      }
    } catch (error) {
  ...
  const handleOptionSelect = (option, slotData) => {
    // เรียกใช้ onSelectOption ที่ส่งมาจาก Modalchill2
    if (onSelectOption && typeof onSelectOption === 'function') {
