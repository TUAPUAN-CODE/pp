...
const ParkingLayoutChill6 = ({ onSlotClick }) => {
  console.log("ParkingLayoutChill6 - ถูกเรียกใช้งาน");
  ...
  const fetchSlots = async () => {
    try {
      console.log("ParkingLayoutChill6 - กำลังดึงข้อมูลช่องจอด");
      const response = await fetch(`${API_URL}/api/coldstorage/room`, { credentials: "include" });
      const data = await response.json();
      
      if (data.slot) {
        const slotData = data.slot.filter((item) => item.cs_id === 5 && item.slot_status);
        const formatted = formatSlotData(slotData);
        
        setColumns(formatted.columns);
        setRows(formatted.rows);
        console.log("ParkingLayoutChill6 - ได้รับข้อมูลช่องจอด:", formatted);
      }
    } catch (error) {
      console.error("ParkingLayoutChill6 - Error fetching slots:", error);
    }
  };
                ...
                return (
                  <td key={colIndex} className="border px-2 py-2 text-center" style={{ backgroundColor: "#fff" }}>
                    {slot && (
                      <div
                        onClick={() => {
                          console.log("ParkingLayoutChill6 - คลิกช่องจอด:", slot);
                          // ตรวจสอบว่าสามารถคลิกได้หรือไม่
                          if ((!slot?.tro_id || slot?.tro_id === "NULL") || isReserved || !slot.slot_status) {
                            console.log("ParkingLayoutChill6 - ไม่สามารถคลิกช่องจอดนี้ได้");
                            return;
                          }
                          // เรียกใช้ onSlotClick ที่ได้รับมา
                          if (onSlotClick && typeof onSlotClick === 'function') {
                            console.log("ParkingLayoutChill6 - เรียกใช้ onSlotClick กับช่องจอด:", slot);
                            onSlotClick(slot);
                          } else {
                            console.log("ParkingLayoutChill6 - ไม่มี onSlotClick หรือไม่ใช่ฟังก์ชัน:", onSlotClick);
                          }
                        }}
                        style={{
                          color: "#686868",
