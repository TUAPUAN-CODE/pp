...
const ParentComponentChill2 = () => {
  ...
  const [showSlotModal, setShowSlotModal] = useState(false); // เพิ่มตัวแปรนี้เหมือน CSR3
  ...
  // ปรับปรุงฟังก์ชัน closeSlotModal อีกครั้ง
  const closeSlotModal = async (action) => {
    ...
    // ตรวจสอบว่าเป็นการกดปุ่มยกเลิกหรือไม่
    if (action === 'CANCEL_ONLY') {
      console.log("📌 CANCEL button was clicked, NOT clearing slot data");
      // ไม่ต้องทำอะไรเพิ่มเติม เพียงแค่ปิดโมดัลและรีเซ็ต processingSlot
      return;
    }
    
    // กรณีอื่นๆ (ไม่ใช่การกดปุ่มยกเลิก) ให้ล้างข้อมูลช่องจอดและส่ง API
