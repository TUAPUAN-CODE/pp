...
const ScanTrolley = ({ open, onClose, onNext, selectedOption, selectedSlot}) => { 
  ...
  const handleOptionSelect = (option, slotData) => {
    console.log("Selected Slot in handleOptionSelect:", slotData); // Debugging
    setSelectedOption(option);
    setSelectedSlot(slotData); // อัปเดต selectedSlot ให้ ScanTrolley
  ...
  const checkTrolleyStatus = async (value) => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/checkTrolley`, {
        params: { tro: value },
      });
  ...
  return (
    <StyledModal open={open} onClose={(e, reason) => {
      ...
      <ModalContent>
        ...
        <Typography>ประเภทการรับเข้า : {selectedOption}</Typography>
        <p style={{color:"#000"}}><strong>Slot ID:</strong> {selectedSlot?.slot_id}</p>
        <p><strong>CS ID:</strong> {selectedSlot?.cs_id}</p>
        <p><strong>Tro ID:</strong> {selectedSlot?.tro_id ? selectedSlot.tro_id : "ไม่มีรถเข็น"}</p>
        <p><strong>Status:</strong> {selectedSlot?.slot_status ? "ว่าง" : "ไม่ว่าง"}</p>
