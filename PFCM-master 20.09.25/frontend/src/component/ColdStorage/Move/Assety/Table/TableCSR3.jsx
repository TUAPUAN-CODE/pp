...
const ParkingCSR3 = ({ onSlotClick, tro_id }) => {
  const [rows, setRows] = useState({});
  const [columns, setColumns] = useState([]);
  const [socket, setSocket] = useState(null);
  const [reservations, setReservations] = useState(new Map()); // Keep track of reservations
  ...
  // Format slot data to organize into rows and columns
  ...
  const fetchSlots = async () => {
    ...
    try {
      ...
      if (data.slot) {
        const slotData = data.slot.filter((item) => item.cs_id === 1 && item.slot_status);
  ...
  useEffect(() => {
    ...
    newSocket.on("slotUpdated", async () => {
      console.log("🔄 Slot update received.");
