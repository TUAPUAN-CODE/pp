...
const Row = ({
  ...
  const oven_to_coldMinutes =
    Math.floor(row.oven_to_cold) * 60 + (row.oven_to_cold % 1) * 100;
  ...
  const { rmfp_id, oven_to_cold, rm_type_id, ...displayRow } = row;
    ...
    <>
      ...
      <TableRow onClick={() => {
        ...
        <CompleteActionCell
          ...
          icon={
            <FaRegCheckCircle style={{ color: "#26c200", fontSize: "20px" }} />
          }
          backgroundColor={backgroundColor} // Add this
...
const TableMainPrep = ({
  ...
  const columns = Object.keys(data[0] || {}).filter(
    (key) => key !== "rmfp_id" && key !== "oven_to_cold"
  );
