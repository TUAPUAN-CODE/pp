...
const Row = ({
  ...
  const { rmfp_id, stay_place, dest, rm_type_id, mapping_id, remark_qc, remark, rm_status, oven_to_cold, ...restRow } = row;
  
  // Create rowDataToDisplay object containing only the fields we want to display
  const rowDataToDisplay = {
    batch: row.batch_after,
    mat: row.mat,
  ...
  return (
    <>
      ...
      <TableRow onClick={() => {
        ...
        sx={{ cursor: 'pointer' }}
      >
        {Object.values(rowDataToDisplay).map((value, idx) => (
          <TableCell
            key={idx}
            align="center"
            style={{
...
const TableMainPrep = ({ handleOpenModal, data, handleRowClick, handleOpenEditModal, handleOpenSuccess }) => {
  ...
  // Define columns based on our specific needs
  const columns = ["batch", "mat", "mat_name", "production", "tro_id", "level_eu", "CookedDateTime"];
